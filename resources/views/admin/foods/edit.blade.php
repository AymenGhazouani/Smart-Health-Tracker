    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 sm:px-6">
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-lg leading-6 font-medium text-gray-900">Edit Food Item</h3>
                    <p class="mt-1 max-w-2xl text-sm text-gray-500">Update the nutritional information for "{{ $food->name }}"</p>
                </div>
                <div class="flex space-x-2">
                    <a href="{{ route('admin.foods.show', $food->id) }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-medium py-2 px-4 rounded-md transition duration-150 ease-in-out">
        ...
        <form action="{{ route('admin.foods.update', $food->id) }}" method="POST" class="px-4 py-5 sm:p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        <div class="text-center">
                            <div class="font-semibold text-yellow-600" id="preview-calories">{{ $food->calories }}</div>
                            <div class="text-gray-500">Calories</div>
                        </div>
                        <div class="text-center">
                            <div class="font-semibold text-blue-600" id="preview-protein">{{ number_format($food->protein, 1) }}g</div>
                            <div class="text-gray-500">Protein</div>
                        </div>
                        <div class="text-center">
                            <div class="font-semibold text-green-600" id="preview-carbs">{{ number_format($food->carbs, 1) }}g</div>
                            <div class="text-gray-500">Carbs</div>
                        </div>
                        <div class="text-center">
                            <div class="font-semibold text-red-600" id="preview-fat">{{ number_format($food->fat, 1) }}g</div>
                ...
                <div class="bg-blue-50 rounded-lg p-4">
                    <h4 class="text-sm font-medium text-gray-900 mb-2">Item Information</h4>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div>
                            <span class="text-gray-500">Created:</span>
                            <span class="font-medium">{{ $food->created_at->format('M d, Y \a\t g:i A') }}</span>
                        </div>
                        <div>
                            <span class="text-gray-500">Last Updated:</span>
                            <span class="font-medium">{{ $food->updated_at->format('M d, Y \a\t g:i A') }}</span>
                        </div>
                    </div>
                </div>
            </div>
