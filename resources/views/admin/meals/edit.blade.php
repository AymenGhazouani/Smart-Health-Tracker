    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 sm:px-6">
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-lg leading-6 font-medium text-gray-900">Edit Meal Plan</h3>
                    <p class="mt-1 max-w-2xl text-sm text-gray-500">Update "{{ $meal->name }}" meal plan</p>
                </div>
                <div class="flex space-x-2">
                    <a href="{{ route('admin.meals.show', $meal->id) }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-medium py-2 px-4 rounded-md transition duration-150 ease-in-out">
        ...
        <form action="{{ route('admin.meals.update', $meal->id) }}" method="POST" class="px-4 py-5 sm:p-6" id="meal-form">
            @csrf
            @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="description" 
                                  ...
                                  placeholder="Describe this meal plan...">{{ old('description', $meal->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="bg-gray-50 rounded-lg p-4">
                        ...
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            <div class="text-center bg-yellow-100 rounded-lg p-3">
                                <div class="font-semibold text-yellow-600" id="total-calories">{{ number_format($meal->total_macros['calories']) }}</div>
                                <div class="text-gray-500">Total Calories</div>
                            </div>
                            <div class="text-center bg-blue-100 rounded-lg p-3">
                                <div class="font-semibold text-blue-600" id="total-protein">{{ number_format($meal->total_macros['protein'], 1) }}g</div>
                                <div class="text-gray-500">Total Protein</div>
                            </div>
                            <div class="text-center bg-green-100 rounded-lg p-3">
                                <div class="font-semibold text-green-600" id="total-carbs">{{ number_format($meal->total_macros['carbs'], 1) }}g</div>
                                <div class="text-gray-500">Total Carbs</div>
                            </div>
                            <div class="text-center bg-red-100 rounded-lg p-3">
                                <div class="font-semibold text-red-600" id="total-fat">{{ number_format($meal->total_macros['fat'], 1) }}g</div>
                    ...
                    <div class="bg-blue-50 rounded-lg p-4">
                        <h4 class="text-sm font-medium text-gray-900 mb-3">Current Foods in Meal</h4>
                        <div class="space-y-2">
                ...
                <div class="space-y-6">
                    <div>
                        <h4 class="text-sm font-medium text-gray-900 mb-3">
                            Update Foods <span class="text-red-500">*</span>
                        </h4>
                        ...
                        <div class="max-h-96 overflow-y-auto border border-gray-200 rounded-md">
                            @foreach($foods as $food)
                                @php
                                    $currentFood = $meal->foods->firstWhere('id', $food->id);
                                    $isSelected = $currentFood !== null;
                                    $currentQuantity = $isSelected ? $currentFood->pivot->quantity : 1;
                                @endphp
                                <div class="food-item p-3 border-b border-gray-100 hover:bg-gray-50" data-food-id="{{ $food->id }}" data-food-name="{{ $food->name }}" data-calories="{{ $food->calories }}" data-protein="{{ $food->protein }}" data-carbs="{{ $food->carbs }}" data-fat="{{ $food->fat }}">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <div class="ml-4 food-quantity-container" style="display: {{ $isSelected ? 'block' : 'none' }};">
                                            <input type="number" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize with existing foods
