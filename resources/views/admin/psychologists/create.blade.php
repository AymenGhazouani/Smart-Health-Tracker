...
@section('page-title', 'Create Psychologist')

@section('admin-content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 sm:px-6">
            <h3 class="text-lg leading-6 font-medium text-gray-900">Add New Psychologist</h3>
            <p class="mt-1 max-w-2xl text-sm text-gray-500">Create a new psychologist profile</p>
        </div>

        <form action="{{ route('admin.psychologists.store') }}" method="POST" class="px-4 py-5 sm:p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="specialty" id="specialty" required
                        ...
                        <option value="Anxiety" {{ old('specialty') == 'Anxiety' ? 'selected' : '' }}>Anxiety</option>
                        <option value="Depression" {{ old('specialty') == 'Depression' ? 'selected' : '' }}>Depression</option>
                        <option value="Trauma" {{ old('specialty') == 'Trauma' ? 'selected' : '' }}>Trauma</option>
                        <option value="Relationship Counseling" {{ old('specialty') == 'Relationship Counseling' ? 'selected' : '' }}>Relationship Counseling</option>
                        <option value="Family Therapy" {{ old('specialty') == 'Family Therapy' ? 'selected' : '' }}>Family Therapy</option>
                        <option value="Child Psychology" {{ old('specialty') == 'Child Psychology' ? 'selected' : '' }}>Child Psychology</option>
                        <option value="Addiction Counseling" {{ old('specialty') == 'Addiction Counseling' ? 'selected' : '' }}>Addiction Counseling</option>
                        <option value="Cognitive Behavioral Therapy" {{ old('specialty') == 'Cognitive Behavioral Therapy' ? 'selected' : '' }}>Cognitive Behavioral Therapy</option>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="bio" id="bio" rows="4"
                              class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('bio') border-red-300 @enderror">{{ old('bio') }}</textarea>
                    @error('bio')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                <div class="flex-1 flex space-x-2">
                                    <input type="time" name="availability[{{ $day }}][0][start]" 
                                           value="{{ old("availability.{$day}.0.start", '09:00') }}"
                                           class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                    <span class="flex items-center text-gray-500">to</span>
                                    <input type="time" name="availability[{{ $day }}][0][end]" 
                                           value="{{ old("availability.{$day}.0.end", '17:00') }}"
                                           class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                </div>
                            </div>
                        @endforeach
