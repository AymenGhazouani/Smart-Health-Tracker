                    ...
                    <div class="bg-white shadow overflow-hidden sm:rounded-lg p-6">
                        <form action="{{ route('appointments.store') }}" method="POST">
                            ...
                            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                ...
                                <!-- Available Slots Field (Will be populated via AJAX) -->
                                <div class="col-span-1">
                                    <label for="availability_slot_id" class="block text-sm font-medium text-gray-700">Available Time Slots</label>
                                    <select name="availability_slot_id" id="availability_slot_id" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('availability_slot_id') border-red-500 @enderror" required disabled>
                                ...
                                <div class="col-span-2">
                                    ...
                                    <textarea name="reason" id="reason" rows="3" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('reason') border-red-500 @enderror">{{ old('reason') }}</textarea>
                                    @error('reason')
                                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                                    @enderror
                                </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const slotSelect = document.getElementById('availability_slot_id');
                ...
                providerSelect.addEventListener('change', function() {
                    ...
                    if (providerId) {
                        ...
                        fetch(`/appointments/get-available-slots?provider_id=${providerId}`)
                            ...
                            .then(slots => {
                                ...
                                if (slots.length === 0) {
                                    ...
                                    slots.forEach(slot => {
                                        ...
                                        option.textContent = `${startTime.toLocaleDateString()} ${startTime.toLocaleTimeString()} - ${endTime.toLocaleTimeString()}`;
                        ...
                        slotSelect.innerHTML = '<option value="">-- Select Provider First --</option>';
                        slotSelect.disabled = true;
                    }
                });
            });
