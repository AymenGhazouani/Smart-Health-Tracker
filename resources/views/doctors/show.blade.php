    ...
    <div class="lg:w-2/3 flex flex-col space-y-6">
        ...
        <div class="bg-white shadow-lg rounded-xl p-6">
            ...
            <p class="text-gray-700">
                {{ $doctor->description ?? 'No description provided for this doctor.' }}
            </p>
        </div>

        <!-- Reviews (same as your previous code) -->
        <div class="bg-white shadow-lg rounded-xl p-6">
                            ...
                            <div class="flex justify-between items-center mb-1">
                                ...
                                @if(auth()->check() && auth()->id() === $review->user_id)
                                    <div class="flex items-center space-x-3">
                                        <a href="{{ route('reviews.edit', $review->id) }}" class="text-blue-500 hover:text-blue-700">✎</a>
                                        <form action="{{ route('reviews.destroy', $review->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-red-500 hover:text-red-700" onclick="return confirm('Are you sure?')">🗑</button>
                                        </form>
                                    </div>
                                @endif
                            </div>
                    ...
                    <div class="flex space-x-1">
                        @for($i = 1; $i <= 5; $i++)
                            <button type="button" @click="rating={{ $i }}" @mouseenter="rating={{ $i }}"
                                class="text-3xl focus:outline-none" :class="rating >= {{ $i }} ? 'text-yellow-400' : 'text-gray-300'">★</button>
                        @endfor
                    </div>
