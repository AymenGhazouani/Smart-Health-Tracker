...
<div class="metrics-form-page">
    <header class="page-header">
        <div class="header-content">
            <h1>Edit Activity</h1>
            <p>Update your activity details</p>
        </div>
        <div class="header-icon">🏃</div>
    </header>

    <div class="form-container">
        <form method="POST" action="{{ route('metrics.activities.update', $activity) }}" class="metrics-form">
            @csrf
            @method('PUT')
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="datetime-local" 
                           id="performed_at" 
                           name="performed_at" 
                           value="{{ old('performed_at', $activity->performed_at->format('Y-m-d\TH:i')) }}" 
                ...
                <div class="form-group">
                    ...
                    <select id="type" 
                        ...
                        <option value="running" {{ old('type', $activity->type) == 'running' ? 'selected' : '' }}>🏃 Running</option>
                        <option value="cycling" {{ old('type', $activity->type) == 'cycling' ? 'selected' : '' }}>🚴 Cycling</option>
                        <option value="swimming" {{ old('type', $activity->type) == 'swimming' ? 'selected' : '' }}>🏊 Swimming</option>
                        <option value="walking" {{ old('type', $activity->type) == 'walking' ? 'selected' : '' }}>🚶 Walking</option>
                        <option value="gym" {{ old('type', $activity->type) == 'gym' ? 'selected' : '' }}>💪 Gym Workout</option>
                        <option value="yoga" {{ old('type', $activity->type) == 'yoga' ? 'selected' : '' }}>🧘 Yoga</option>
                        <option value="dancing" {{ old('type', $activity->type) == 'dancing' ? 'selected' : '' }}>💃 Dancing</option>
                        <option value="hiking" {{ old('type', $activity->type) == 'hiking' ? 'selected' : '' }}>🥾 Hiking</option>
                        <option value="sports" {{ old('type', $activity->type) == 'sports' ? 'selected' : '' }}>⚽ Sports</option>
                        <option value="other" {{ old('type', $activity->type) == 'other' ? 'selected' : '' }}>🏃‍♂️ Other</option>
                    </select>
                    @error('type')
                        <p class="error-message">{{ $message }}</p>
                    @enderror
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <input type="number" 
                           ...
                           min="0" 
                           step="0.01" 
                           value="{{ old('distance_km_times100', $activity->distance_km_times100) }}" 
            ...
            <div class="form-group">
                ...
                <textarea id="note" 
                          ...
                          placeholder="Add any notes about this activity (e.g., intensity, how you felt, equipment used, etc.)">{{ old('note', $activity->note) }}</textarea>
                @error('note')
                    <p class="error-message">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="preview-card">
                ...
                <div class="preview-content">
                    <div class="preview-item">
                        ...
                        <span class="preview-value" id="preview-datetime">{{ $activity->performed_at->format('M j, Y g:i A') }}</span>
                    </div>
                    <div class="preview-item">
                        <span class="preview-label">Activity:</span>
                        <span class="preview-value" id="preview-type">
                            @php
                                $activityEmojis = [
                                    'running' => '🏃',
                                    'cycling' => '🚴',
                            ...
                            @endphp
                            {{ $activityEmojis[$activity->type] ?? '🏃‍♂️' }} {{ ucfirst($activity->type) }}
                        </span>
                    </div>
                    <div class="preview-item">
                        ...
                        <span class="preview-value" id="preview-duration">
                            ...
                            @endphp
                            {{ $hours > 0 ? $hours.'h '.$minutes.'m' : $minutes.'m' }} ({{ $activity->duration_minutes }} min)
                        </span>
                    </div>
                    <div class="preview-item">
                        <span class="preview-label">Distance:</span>
                        <span class="preview-value" id="preview-distance">
                            {{ $activity->distance_km_times100 ? number_format($activity->distance_km_times100/100, 2).' km' : '--' }}
                        </span>
                    </div>
                    <div class="preview-item">
                        <span class="preview-label">Calories:</span>
                        <span class="preview-value" id="preview-calories">
                            {{ $activity->calories ? $activity->calories.' kcal' : '--' }}
                        </span>
                    </div>
                    <div class="preview-item">
                        <span class="preview-label">Note:</span>
                        <span class="preview-value" id="preview-note">{{ $activity->note ?: '--' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="metadata-card">
                ...
                <div class="metadata-content">
                    <div class="metadata-item">
                        ...
                        <span class="metadata-value">{{ $activity->created_at->format('M j, Y g:i A') }}</span>
                    </div>
                    <div class="metadata-item">
                        <span class="metadata-label">Last Updated:</span>
                        <span class="metadata-value">{{ $activity->updated_at->format('M j, Y g:i A') }}</span>
                    </div>
                </div>
            </div>
