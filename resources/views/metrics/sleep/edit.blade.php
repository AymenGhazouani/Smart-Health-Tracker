...
@section('title', 'Edit Sleep Session')

@section('content')
<div class="metrics-form-page">
    <header class="page-header">
        <div class="header-content">
            <h1>Edit Sleep Session</h1>
            <p>Update your sleep session</p>
        </div>
        <div class="header-icon">😴</div>
    </header>

    <div class="form-container">
        <form method="POST" action="{{ route('metrics.sleep.update', $sleepSession) }}" class="metrics-form">
            @csrf
            @method('PUT')
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="datetime-local" 
                           id="started_at" 
                           name="started_at" 
                           value="{{ old('started_at', $sleepSession->started_at->format('Y-m-d\TH:i')) }}" 
                ...
                <div class="form-group">
                    ...
                    <input type="datetime-local" 
                           id="ended_at" 
                           name="ended_at" 
                           value="{{ old('ended_at', $sleepSession->ended_at->format('Y-m-d\TH:i')) }}" 
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <select id="quality" 
                        ...
                        @for($i = 1; $i <= 10; $i++)
                            <option value="{{ $i }}" {{ old('quality', $sleepSession->quality) == $i ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <input type="number" 
                           ...
                           min="1" 
                           max="1440" 
                           value="{{ old('duration_minutes', $sleepSession->duration_minutes) }}" 
            ...
            <div class="form-group">
                ...
                <textarea id="note" 
                          ...
                          placeholder="Add any notes about this sleep session (e.g., had trouble falling asleep, woke up refreshed, etc.)">{{ old('note', $sleepSession->note) }}</textarea>
                @error('note')
                    <p class="error-message">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="preview-card">
                ...
                <div class="preview-content">
                    <div class="preview-item">
                        ...
                        <span class="preview-value" id="preview-start">{{ $sleepSession->started_at->format('M j, Y g:i A') }}</span>
                    </div>
                    <div class="preview-item">
                        <span class="preview-label">Wake Time:</span>
                        <span class="preview-value" id="preview-end">{{ $sleepSession->ended_at->format('M j, Y g:i A') }}</span>
                    </div>
                    <div class="preview-item">
                        <span class="preview-label">Duration:</span>
                        <span class="preview-value" id="preview-duration">{{ floor($sleepSession->duration_minutes / 60) }}h {{ $sleepSession->duration_minutes % 60 }}m ({{ $sleepSession->duration_minutes }} min)</span>
                    </div>
                    <div class="preview-item">
                        <span class="preview-label">Quality:</span>
                        <span class="preview-value" id="preview-quality">
                            @if($sleepSession->quality)
                                {{ $sleepSession->quality }}/10 ({{ $sleepSession->quality <= 3 ? 'Poor' : ($sleepSession->quality <= 6 ? 'Fair' : ($sleepSession->quality <= 8 ? 'Good' : 'Excellent')) }})
                            @else
                                --
                            @endif
                        </span>
                    ...
                    <div class="preview-item">
                        ...
                        <span class="preview-value" id="preview-note">{{ $sleepSession->note ?: '--' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="metadata-card">
                ...
                <div class="metadata-content">
                    <div class="metadata-item">
                        ...
                        <span class="metadata-value">{{ $sleepSession->created_at->format('M j, Y g:i A') }}</span>
                    </div>
                    <div class="metadata-item">
                        <span class="metadata-label">Last Updated:</span>
                        <span class="metadata-value">{{ $sleepSession->updated_at->format('M j, Y g:i A') }}</span>
                    </div>
                </div>
            </div>
